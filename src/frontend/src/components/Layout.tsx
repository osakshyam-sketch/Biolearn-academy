...
function MuteButton() {
  ...
  return (
    <button
      ...
      className="flex h-8 w-8 items-center justify-center rounded-lg border border-border bg-background text-base transition-colors hover:bg-muted focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
      style={{ outlineColor: "oklch(0.5 0.15 35)" }}
    >
...
function LayoutInner({ children }: LayoutProps) {
  ...
  const activeColor = activeSection
    ? (colorMap[activeSection] ?? "oklch(0.46 0.13 155)")
    : "oklch(0.46 0.13 155)";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Header */}
      <header
        className="fixed top-0 right-0 z-20 flex items-center justify-between px-6 py-3 border-b border-border bg-card shadow-sm lg:left-64"
        style={{ left: 0 }}
        data-ocid="header"
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span
              className="mx-0.5 h-3 w-px bg-border opacity-60"
              aria-hidden="true"
            />
        ...
        {/* Right side: active section indicator + mute toggle */}
        <div className="flex items-center gap-3">
          <div
      ...
      {/* Footer */}
      <footer className="lg:pl-64 border-t border-border py-4 px-6 text-center text-xs text-muted-foreground bg-muted/40">
